...
//    var_dump($order);die;				//added by zh.t for test log
    ...
    //added by zh.t for 内部商品只能用余额支付 start
    /* 如果有内部商品，则删除除余额支付以外的所有支付方式 */
    if ((!isset($_CFG['use_surplus']) || $_CFG['use_surplus'] == '1')
    && $_SESSION['user_id'] > 0)
    {
    	foreach ($cart_goods AS $val)
    	{
    		$sql_2 = "SELECT * FROM " . $ecs->table('goods') . " WHERE goods_id = " . $val['goods_id'];
    		$r = $GLOBALS['db']->getRow($sql_2);
...
//    	var_dump($payment_list);die;				//added by zh.t
    	
    }
    //added by zh.t for 内部商品只能用余额支付 end
...
//    var_dump($user_info);die;				//added by zh.t for test
    ...
    //added by zh.t for 购物币 start
    /* 如果使用购物币，取得购物币 */    
    if ((!isset($_CFG['use_surplus']) || $_CFG['use_surplus'] == '1')
    && $_SESSION['user_id'] > 0)
    {    	
    	$allow_shopping_money = 1;
    	foreach ($cart_goods AS $val)
    	{
    		$sql_2 = "SELECT * FROM " . $ecs->table('goods') . " WHERE goods_id = " . $val['goods_id'];
    		$r = $GLOBALS['db']->getRow($sql_2);
    		...
    		if($r['cat_id'] != 4)
    		{
    			$allow_shopping_money = 0;
    			break;
    		}
    	}
    	
    	$smarty->assign('allow_use_shopping_money', $allow_shopping_money);
    	$smarty->assign('your_shopping_money', $user_info['shopping_money']);
    	$smarty->assign('your_other_integral', $user_info['other_points']);
    }
    //added by zh.t for 购物币 end
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $order = array(
    	...
    	'shopping_money'         => isset($_POST['shopping_money']) ? floatval($_POST['shopping_money']) : 0.00,
        'integral'        => isset($_POST['integral']) ? intval($_POST['integral']) : 0,
    	'other_integral'        => isset($_POST['other_integral']) ? intval($_POST['other_integral']) : 0,
...
//    var_dump($order);die;			//added by zh.t for test
 ...
 //       var_dump($flow_points . "----" . $user_points . "----" . $order['integral']);die;				//added by zh.t
        ...
        //added by zh.t for 查询用户的other_integral start
        $order['shopping_money'] = min($order['shopping_money'], $user_info['shopping_money']);
        if ($order['shopping_money'] < 0)
        {
        	$order['shopping_money'] = 0;
        }
        //added by zh.t for 查询用户的other_integral end
        
        //added by zh.t for 查询用户的other_integral start
        $order['other_integral'] = min($order['other_integral'], $user_info['other_points']);
        if ($order['other_integral'] < 0)
        {
        	$order['other_integral'] = 0;
        }
        //added by zh.t for 查询用户的other_integral end        
    }
    else
...
// 	var_dump($order['shipping_fee'] . "----" . $order['insure_fee']);die;					//added by zh.t
    ...
    if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0)
    {
        ...
        if ($order['order_amount'] > ($user_info['user_money'] + $user_info['credit_line'] + $order['shopping_money'] + $order['integral']/100 + $order['other_integral']))					//modified by zh.t
        {
 ...
 //       	$order['surplus'] = $order['order_amount'];
            $order['surplus'] = $order['order_amount'] - $order['shopping_money'] - $order['integral']/100 - $order['other_integral'];				//modified by zh.t
    ...
    //added by zh.t for 购物币 start
    if ($order['user_id'] > 0 && ($order['shopping_money'] > 0 || $order['other_integral'] > 0))
    {
    	$userid = $order['user_id'];
    	$shopping_money = $order['shopping_money'] * -1;
    	$other_integral = $order['other_integral'] * -1;
    	$localTime = gmtime();
    	
    	$sql = "INSERT INTO " .$GLOBALS['ecs']->table('shopping_money_log'). " (user_id, shopping_money, change_time)" .
    			"VALUES ('$userid', '$shopping_money', '$localTime')";
    	$GLOBALS['db']->query($sql);
    	
    	$sql = "UPDATE " . $GLOBALS['ecs']->table('users') ." SET shopping_money = shopping_money + '$shopping_money',other_points = other_points + '$other_integral' " . " WHERE user_id = '$userid' LIMIT 1";    	
    	$GLOBALS['db']->query($sql);    	
    }
    //added by zh.t for 购物币 end
